...
const SetPickup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await db.collection('route').doc("1").set({
        route: route
      });
  ...
  return (
    <div className="d-flex">
      ...
      <div className="ps-4 pt-3 container-fluid">
        ...
        <h1>
          Clicked Location:{" "}
          {clickedLocation ? `${clickedLocation.lat}, ${clickedLocation.lng}` : "None"}
        </h1>
        <div className="map-container" style={{ height: "calc(100vh - 200px)" }} ref={mapRef}></div>
      </div>
    </div>
  );
};
